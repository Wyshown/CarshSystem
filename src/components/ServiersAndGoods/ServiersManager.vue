<template>
  <div class="main-container">
    <h5 class="title-h4"><span class="zdy-span" @click="add"><i class="fa fa-plus"></i>&nbsp; 新增</span><p>服务项目</p></h5>

    <div class="row padding-left-10 form-horizontal">
      <div class="form-group col-xs-12  col-sm-12 col-md-5 col-lg-5">
        <!--<label  class="col-xs-0 col-sm-0 col-md-4 col-lg-4 control-label rmpaddingR padding-left-0 text-center">所属类目：</label>-->
        <!--<div class="col-xs-12 col-sm-12 col-md-8 col-lg-8">-->
          <!--<select v-model="serviceSelected" class="form-control">-->
            <!--<option value="">请选择服务类目</option>-->
...
<script type="text/ecmascript-6">
  ...
  export default {
    mounted () {
      this.headerList = [{name: '一级类目'}, {name: '二级类目'}, {name: '服务名称'}, {name: '销售状态'}, {name: '定价类型'}, {name: '服务标价'}, {name: '银卡返积分比例'}, {name: '金卡返积分比例'}, {name: '铂金卡返积分比例'}, {name: '特惠卡返积分比例'}, {name: '操作'}]
    },
    ...
    methods: {
        ...
        api.request('post', 'index/getGoodsList', _params)
          .then(response => {
            ...
            for (let index in response.data.data.info) {
              ...
              let item = {name: dic.p_type_name, cate: dic.type_name, tdata3: dic.goods_name, tdata4: this.returnSaleState(dic.goods_type), tdata5: dic.goods_price_type === '1' ? '固定价格' : '非固定价格', tdata6: dic.goods_price, tdata7: dic.integral_ratio.length > 0 ? parseInt(parseFloat(dic.integral_ratio[0].proportion) * 100) + '%' : '暂无', tdata8: dic.integral_ratio.length > 1 ? parseInt(parseFloat(dic.integral_ratio[1].proportion) * 100) + '%' : '暂无', tdata9: dic.integral_ratio.length > 2 ? parseInt(parseFloat(dic.integral_ratio[2].proportion) * 100) + '%' : '暂无', tdata10: dic.integral_ratio.length > 3 ? parseInt(parseFloat(dic.integral_ratio[3].proportion) * 100) + '%' : '暂无'}
              item = {...item, 'tdata11': {str: this.returnBtnArr(dic.goods_type, dic.shopGoodsId), isBtn: true}}
              arr.push(item)
            }
            this.bodyList = arr
            this.serId = ''
      ...
      edit (item) {
        this.$router.push({name: 'ServiersDetail', params: {type: 'edit', 'id': item.str[0].id}})
      },
      del (item) {
      ...
      detail (item) {
        this.$router.push({name: 'ServiersAndGoodsDetail', params: {type: 'serviers', 'id': item.str[0].id}})
      },
      add () {
        this.$router.push({name: 'ServiersDetail', params: {type: 'add', 'id': '0'}})
      }
    },
    components: {
      Tables,
